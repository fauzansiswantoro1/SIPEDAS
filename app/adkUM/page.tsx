...
import { Upload, FileSpreadsheet, AlertCircle, Search, Download } from "lucide-react"
...
export default function ADKUangMakanPage() {
  ...
  const getEmployeeGrade = (nama: string): EmployeeGrade | undefined => {
    return employeeGrades.find((emp) => emp.nama.toLowerCase() === nama.toLowerCase())
  }

  const calculateRateAndTax = (golongan: string): { rate: number; taxRate: number } => {
    let rate = 0
    let taxRate = 0
  ...
  function calculateUangMakan(): CalculationResult[] {
    ...
    dataRows.forEach((row: any[], rowIndex: number) => {
      ...
      console.log(`[v0] Processing row ${rowIndex + 1}: Nama='${nama}', NIP='${nip}'`)

      const employeeGrade = getEmployeeGrade(nama)

      if (!employeeGrade) {
        console.warn(`[v0] Skipping employee with Nama: '${nama}' (NIP: '${nip}') - Name not found in grade data.`)
        return
      }
  ...
  const totalCalculationPages = Math.ceil(filteredCalculationData.length / itemsPerCalculationPage)
  ...
  const loadTunjanganKinerja = async () => {
    try {
      const supabase = createClient()
      const { data, error } = await supabase.from("tunjangan_kinerja").select("*").order("nama", { ascending: true })
  ...
  const handleDownloadTxt = async (employeeType: "CPNS" | "PNS" | "PPPK") => {
    ...
    dataRows.forEach((row: any[]) => {
      ...
      if (nip && (nip.startsWith("TATT") || nip.startsWith("PPNPN") || nip.startsWith("DIRDATA"))) {
        console.log(`[v0] Skipping employee with NIP: ${nip} (excluded prefix)`)
        return
      }
      ...
      if (nip) {
        ...
        if (employeeType === "PNS" && (nip.includes("2025") || nip.includes("2024"))) {
      ...
      dateCols.forEach((col: string) => {
        ...
        const value = row[colIndex]?.toString().toUpperCase().trim()
        if (value === "WFO") {
          const date = `${selectedYear}-${selectedMonth.padStart(2, "0")}-${col.padStart(2, "0")}`
    ...
    const generatedFileName = `WFO_${employeeType}_${selectedYear}${selectedMonth}_${fileName.replace(/\.[^/.]+$/, "")}.txt`
      ...
      if (user) {
        ...
        if (existingFiles && existingFiles.length > 0) {
          const relevantCalculations = calculationResults.filter((calc) => {
            ...
            if (employeeType === "PNS") return !nip.includes("2025") && !nip.includes("2024")
            return false
          })

          const calculationData = relevantCalculations.reduce(
            (acc, calc) => {
              acc[calc.nip] = {
                nama: calc.nama,
        ...
        if (shouldSave) {
          ...
          // Insert new record
          const { error: insertError } = await supabase.from("adk_archives").insert({
          ...
          if (insertError) {
            ...
            alert("File generated but failed to save to archive: " + insertError.message)
          } else {
  ...
  const handleConfirmReplace = async () => {
    ...
    try {
      ...
      if (updateError) {
        ...
        alert("File generated but failed to update archive: " + updateError.message)
      } else {
    ...
    // Download the file
    const blob = new Blob([confirmDialogData.fileContent], { type: "text/plain" })
    const url = URL.createObjectURL(blob)
    const link = document.createElement("a")
    link.href = url
  ...
  const downloadCalculationResults = () => {
    ...
    try {
      ...
      const filename = `uang_makan_calculation_${selectedYear}${selectedMonth}_${new Date().toISOString().split("T")[0]}.xlsx`
  ...
  return (
    <div className="min-h-screen relative">
          ...
          <div className="bg-background border border-border rounded-xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="bg-gradient-to-r from-amber-500 to-orange-500 p-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-xl font-bold text-white">File Already Exists</h3>
                  <p className="text-white/90 text-sm">Duplicate ADK file detected</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-2">
                <p className="text-foreground">
                  An ADK .txt file already exists in the archive with the following details:
                </p>
                <div className="bg-muted/50 border border-border rounded-lg p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Employee Type:</span>
                    <span className="text-sm font-semibold text-foreground">{confirmDialogData.employeeType}</span>
                  </div>
                  <div className="flex items-center justify-between">
                  ...
                  <div className="pt-2 border-t border-border">
                    <span className="text-sm text-muted-foreground">Existing File:</span>
              ...
              <p className="text-sm text-muted-foreground">
                Do you want to replace the existing file with the new one, or cancel and keep the current file?
              </p>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-foreground mb-2">ADK Uang Makan</h2>
          <p className="text-muted-foreground">Upload and process attendance data for uang makan calculation</p>
        </div>
          ...
          <Card className="border-border shadow-sm">
            ...
            <CardContent className="p-6">
              <div className="mb-6 p-4 bg-muted/30 border border-border rounded-lg">
                <h3 className="text-sm font-semibold text-foreground mb-3">Select Period for ADK Generation</h3>
                ...
                {filteredRows.length > 0 && (
                  <div className="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="text-sm text-muted-foreground">
                      Showing {filteredRows.length} rows × {headers.length} columns
                      {filteredRows.length < rows.length && (
                        <span className="ml-2 text-amber-600">
                          ({rows.length - filteredRows.length} rows filtered out)
                        </span>
                      )}
                    </div>
                ...
                <div className="flex flex-col space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>
                        Uang makan calculation ({filteredCalculationData.length} records)
                      </CardDescription>
                    </div>
                    <Button
              ...
              <CardContent className="p-0">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="hover:bg-transparent border-b border-border">
                        ...
                        <TableHead className="font-semibold text-foreground">Tax (5%)</TableHead>
                    ...
                    <TableBody>
                      {paginatedCalculationData.map((calc, index) => (
                        <TableRow key={index} className="border-b border-border hover:bg-muted/50">
                          ...
                          <TableCell>Rp {calc.baseAmount.toLocaleString()}</TableCell>
                          <TableCell>Rp {calc.taxAmount.toLocaleString()}</TableCell>
                          <TableCell className="font-semibold">Rp {calc.totalUangMakan.toLocaleString()}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                ...
                {totalCalculationPages > 1 && (
                  <div className="flex items-center justify-between px-6 py-4 border-t border-border">
                    <div className="text-sm text-muted-foreground">
                      Showing {(currentCalculationPage - 1) * itemsPerCalculationPage + 1} to{" "}
                      {Math.min(currentCalculationPage * itemsPerCalculationPage, filteredCalculationData.length)} of{" "}
                      {filteredCalculationData.length} entries
                    </div>
                    <div className="flex items-center space-x-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setCurrentCalculationPage((prev) => Math.max(1, prev - 1))}
                      ...
                      <div className="flex items-center space-x-1">
                        {Array.from({ length: Math.min(5, totalCalculationPages) }, (_, i) => {
                          let pageNum
                          if (totalCalculationPages <= 5) {
                            pageNum = i + 1
                          } else if (currentCalculationPage <= 3) {
                            pageNum = i + 1
                          } else if (currentCalculationPage >= totalCalculationPages - 2) {
                          ...
                          return (
                            <Button
                              key={pageNum}
                              variant={currentCalculationPage === pageNum ? "default" : "outline"}
                              size="sm"
                              onClick={() => setCurrentCalculationPage(pageNum)}
                              className={
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setCurrentCalculationPage((prev) => Math.min(totalCalculationPages, prev + 1))}
                        disabled={currentCalculationPage === totalCalculationPages}
                        className="border-border hover:bg-muted"
                      >
                        Next
                      </Button>
